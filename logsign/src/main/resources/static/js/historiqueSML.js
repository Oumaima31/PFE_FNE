...
function renderTable() {
  ...
  // Modifier l'en-tête du tableau pour changer "Action" en "Statut"
  const tableHeaders = document.querySelectorAll("#historique-table thead th");
  if (tableHeaders.length >= 3) {
    tableHeaders[2].textContent = "Statut";
  }
...
function createFNEPdfView(fne) {
  ...
  // Créer un nouveau modal pour la vue PDF
  const fnePdfModal = document.createElement('div');
  fnePdfModal.id = "fnePdfModal";
  fnePdfModal.className = "modal pdf-modal";
...
function closeFnePdfModal() {
  ...
  if (fnePdfModal) {
    document.body.removeChild(fnePdfModal);
  }
}
